...
public class UIUnlockWeapon : BaseMonoBehaviour
{
	...
	private void OnChangeSelection(Selectable NewSelectable, Selectable PrevSelectable)
	{
		...
		damageText.text = string.Format(damage + ": {0}{1}</color>", "<color=#2DFF1C>", num);
		speedText.text = string.Format(speed + ": {0}{1}</color>", "<color=#2DFF1C>", num2);
		num = Mathf.Round(num * 100f) / 100f;
		num2 = Mathf.Round(num2 * 100f) / 100f;
		upgradeDescriptionText.text = "";
		upgradeDescriptionText.text = LocalizationManager.GetTranslation("UpgradeSystem/WeaponDamageIncrease") + " <color=#2DFF1C>" + num + "</color>\n";
		TMP_Text tMP_Text = upgradeDescriptionText;
		tMP_Text.text = tMP_Text.text + LocalizationManager.GetTranslation("UpgradeSystem/WeaponSpeedIncrease") + " <color=#2DFF1C>" + num2;
