...
public class WeaponShopKeeperManager : MonoBehaviour
{
	...
	[SerializeField]
	private Interaction_SimpleConversation relicRiddleConvo;
	...
	private void Start()
	{
		...
		{
			relicRiddleConvo.OnInteraction += delegate
			{
				ObjectiveManager.Add(new Objectives_Custom("Objectives/GroupTitles/KudaiiRelic", Objectives.CustomQuestTypes.FindKudaiiRelic), true);
			};
		}
	}
	...
	{
		relicRiddleConvo.ActivateDistance = 1.25f;
		relicRiddleConvo.AutomaticallyInteract = false;
		GameManager.GetInstance().WaitForSeconds(1f, delegate
		{
			relicRiddleConvo.Finished = false;
			relicRiddleConvo.Spoken = false;
		});
	}

	private bool CheckWeaponIsntUsed(EquipmentType type)
	{
		foreach (Interaction_WeaponItem weaponSlot in WeaponSlots)
		{
	...
	private int GetWeaponCost(EquipmentType type, int level)
	{
		...
		return num + Mathf.Clamp((int)((float)(level - DataManager.Instance.CurrentRunWeaponLevel) * 1.5f), 0, 5);
	}
