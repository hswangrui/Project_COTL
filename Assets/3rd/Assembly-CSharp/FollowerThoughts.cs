...
public class FollowerThoughts
{
	...
	{
		return LocalizationManager.GetTranslation(string.Format("FollowerThoughts/{0}", Type));
	}

	public static string GetLocalisedDescription(Thought Type)
	{
		return LocalizationManager.GetTranslation(string.Format("FollowerThoughts/{0}/Description", Type));
	}

	public static string GetNotificationOnLocalizationKey(Thought type)
	{
		return string.Format("Notifications/{0}/Notification/On", type);
	}

	public static string GetNotificationOffLocalizationKey(Thought type)
	{
		return string.Format("Notifications/{0}/Notification/Off", type);
	}
	...
	public static ThoughtData GetData(Thought thought)
	{
		...
		case Thought.Cult_DiedInDungeon:
			return new ThoughtData(thought)
			{
				Modifier = ((DifficultyManager.AssistModeEnabled && DifficultyManager.SecondaryDifficulty == DifficultyManager.Difficulty.Easy) ? (-5) : (-10)),
				Duration = 6000f
			};
