...
{
	public static Action OnRitualFastingBegan;
	...
	private IEnumerator RitualRoutine()
	{
		...
		sermonEffect.AnimationState.AddAnimation(0, "5-loop", true, 0f);
		SermonEffectStartingPos = sermonEffect.transform.position;
		...
		foreach (FollowerBrain item in list)
		{
			...
			{
				((FollowerTask_AttendRitual)item.CurrentTask).Pray();
			}
		}
		yield return new WaitForSeconds(7f);
		sermonEffect.AnimationState.AddAnimation(0, "5-out", false, 0f);
		...
		sermonEffect.transform.position = SermonEffectStartingPos;
		...
		foreach (FollowerBrain item2 in Ritual.FollowerToAttendSermon)
		{
			...
			{
				(item2.CurrentTask as FollowerTask_AttendRitual).Boo();
			}
		}
		...
		Action onRitualFastingBegan = OnRitualFastingBegan;
		if (onRitualFastingBegan != null)
		{
			onRitualFastingBegan();
		}
		CompleteRitual();
		yield return new WaitForSeconds(1f);
		CultFaithManager.AddThought(Thought.Cult_Fast, -1, 1f);
	}
	...
	private IEnumerator MoveFollower(Follower follower, int index)
	{
		...
		follower.SetBodyAnimation("Hungry/get-hungry", false);
		follower.AddBodyAnimation("Hungry/idle-hungry", true, 0f);
	}
}
