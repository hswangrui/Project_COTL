...
namespace Beffio.Dithering
{
	...
	public class Stylizer : StylizerBase
	{
        ...
        [SerializeField] private Palette _palette2;


        [SerializeField] public bool UseSecondPalette;

        [Range(0f, 1f)]
        [Tooltip("Lerp between two palettes.")]
        [SerializeField]
        private float _lerpPalette;


        public Palette Palette { get { return _palette; } set { _palette = value; } }


        public Palette Palette2 { get { return _palette2; } set { _palette2 = value; } }

        public float LerpPalette { get { return _lerpPalette; } set { _lerpPalette = value; } }
		...
		private void DitherRender(RenderTexture source, RenderTexture destination){
				...
				Material.SetTexture("_PaletteTex2", _palette2.Texture);
				...
				if (UseSecondPalette)
				{
					Material.EnableKeyword("_USESECONDPALETTE");
				}
				else
				{
					Material.DisableKeyword("_USESECONDPALETTE");
				}
				Material.SetFloat("_PaletteLerp", _lerpPalette);
				Material.SetFloat("_PatternSize", patTex.width);
				Material.SetTexture("_PatternTex", patTex);
