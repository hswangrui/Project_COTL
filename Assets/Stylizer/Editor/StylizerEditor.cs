...
namespace Beffio.Dithering
{
	...
	public class StylizerEditor : Editor 
	{
		...
		private static class ContentText
		{
			...
			public static GUIContent paletteProperty2 = new GUIContent("Palette Asset2", "Asset - The colors which can appear in the dithering effect, and how many colors can be mixed");
		...
		SerializedProperty _paletteProperty2;
		SerializedProperty _lerpPaletteProperty;
		...
		void OnEnable(){
			...
			_paletteProperty2 = serObj.FindProperty("_palette2");

			_lerpPaletteProperty = serObj.FindProperty("_lerpPalette");
		...
		private void DrawGUIFields()
		{
			...
			if(Dither.boolValue) {
				...
				EditorGUILayout.PropertyField(_paletteProperty2, ContentText.paletteProperty2);
				 EditorGUILayout.Slider(_lerpPaletteProperty, 0, 1f);
		...
		private void DrawGUIWarning()
		{
			...
			if (Effect.Palette2 != null)
			{
				if (!Effect.Palette2.HasTexture)
				{
					warningMessage = string.Format(ContentText.noTextureWarning, warningMessage, typeof(Pattern).Name, Effect.Palette2.name);
				}
				if (Effect.Palette2.IsDirty)
				{
					warningMessage = string.Format(ContentText.oldTextureWarning, warningMessage, typeof(Pattern).Name, Effect.Palette2.name);
				}
			}
